  ...
  return <><ScrollArea className="h-full scrollbar-hidden"><div className="space-y-3 p-3">
    ...
    {tab === 'workflow' ? <>
      ...
      <div className="grid grid-cols-2 gap-2">
        <PropLabel label={t('Nhận dạng', 'Recognition')}><select className="h-8 w-full rounded-md border border-border bg-background px-2 text-xs" value={settings.engine} disabled={busy} onChange={(e) => selectRecognitionEngine(e.target.value as ProjectSettings['engine'])}><option value="whisper">Whisper · {t('chất lượng', 'quality')}</option><option value="paddleocr">OCR</option><option value="subtitle">SRT</option></select></PropLabel>
