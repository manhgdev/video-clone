...
@app.post("/api/tts/studio/synthesize")
def api_tts_studio_synth(body: StudioSynthIn):
    """TTS Studio: synth full text → data/tts_output/{jobId}/."""
    text = (body.text or "").strip()
    if not text:
        raise HTTPException(400, "Thiếu nội dung")
    ensure_vieneu_dirs()
    ...
    meta = {
        "id": job_id,
        "title": (body.title or text[:48]).strip(),
        "voice": body.voice,
        "lang": body.lang,
        "duration": dur,
        "engine": (
            "vieneu"
            if (body.voice or "").startswith("vn:")
            else "capcut"
            if (body.voice or "").startswith("cc:")
            else "elevenlabs"
            if (body.voice or "").startswith("el:")
            else "system"
        ),
...
@app.get("/api/tts/studio/history")
def api_tts_studio_history():
    ...
    for d in sorted(TTS_OUTPUT.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True):
        ...
        try:
            ...
            m["audioUrl"] = f"/api/tts/studio/jobs/{d.name}/audio.wav"
            items.append(m)
        except Exception:
            continue
        if len(items) >= 50:
...
@app.get("/api/tts/studio/jobs/{job_id}/audio.wav")
def api_tts_studio_audio(job_id: str):
...
@app.delete("/api/tts/studio/jobs/{job_id}")
...
async def api_tts_studio_clone(
    ...
    try:
        ...
        voice = vieneu_engine.clone_voice(name.strip(), tmp)
    except Exception as e:
        raise HTTPException(500, str(e)) from e
    finally:
