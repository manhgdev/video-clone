...
_el_key_i = 0
_el_voices_cache: list[dict[str, str]] | None = None
...
def _el_tts(text: str, voice_id: str, out_wav: Path, lang: str | None = None) -> None:
    ...
    if not keys:
        raise RuntimeError(
            ...
            "hoặc set ELEVENLABS_API_KEYS trong server/.env"
        )
...
def _el_voice_options() -> list[dict[str, str]]:
    ...
    preset = [{"id": f"el:{vid}", "name": f"ElevenLabs · {name}"} for vid, name in _EL_PRESET]
    keys = _el_keys()
    if not keys:
        _el_voices_cache = []
        return []
    try:
        ...
        if r.status_code < 400:
            ...
            for v in r.json().get("voices", []):
                ...
                if vid:
                    fetched.append({"id": f"el:{vid}", "name": f"ElevenLabs · {name}"})
            if fetched:
